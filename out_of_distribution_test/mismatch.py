...
def sim_policy(variant, path_to_exp, num_trajs=1, deterministic=False, save_video=False, mpc_only=False, save_path=None):
    ...
    print('testing on {} test tasks, {} trajectories each'.format(env.num_tasks_out_of_distribution, num_trajs))
    ...
    col_free_ratio = num_col_free / len(eval_tasks)

    print('collision-free ratio : ', col_free_ratio)
    print('average computation time : ', np.mean(elapsed_list))
    print('reach ratio : ', len(reaches) / env.num_tasks_out_of_distribution)
...
def main_external_call(config, path, num_trajs, deterministic, video, mpc_only, save_path):
    ...
    if config:
        with open(osp.join(config)) as f:
            exp_params = json.load(f)
        variant = deep_update_dict(exp_params, variant)
    mean_elapsed_time, col_free_ratio = sim_policy(variant, path, num_trajs, deterministic, video, mpc_only, save_path)
    return mean_elapsed_time, col_free_ratio
