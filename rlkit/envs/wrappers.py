...
class ProxyEnv(Serializable, Env):
    ...
    def step(self, action, train=True):
        return self._wrapped_env.step(action,train)
...
class NormalizedBoxEnv(ProxyEnv, Serializable):
    ...
    def step(self, action, train=True, Xout=None, use_MPC=False, init_state=None):
        ...
        wrapped_step = self._wrapped_env.step(scaled_action, train=train, Xout=Xout, init_state=init_state)
        next_obs, reward, done, action, col, info_custom, info = wrapped_step
        normalized_obs = next_obs / 5.      # all entries lie in [-1, 1]
        normalized_obs[2] *= 5. / np.pi

        action = 2. * (action - lb) / (ub - lb) - 1.

        if self._should_normalize:
            next_obs = self._apply_normalize_obs(next_obs)
        return next_obs, reward * self._reward_scale, done, action, col, info_custom, info, normalized_obs
